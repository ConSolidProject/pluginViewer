  ...
  class ModelViewer extends Component {
    ...
    componentDidUpdate(prevProps) {
      ...
      if (prevProps.models !== currentProps.models) {
        ...
        toRemove.forEach((el) => {
          const elID = el.id;
          console.log(`this.viewer.scene.models`, this.viewer.scene.models)
            const elToRemove = this.viewer.scene.models[elID];
                      if (elToRemove) { //JW added if statement to avoid error => maybe remove if lib is understood better?
      ...
      scene.input.on(eventToPickOn, (coords) => {
        ...
        if (hit) {
          ...
          if (this.props.onSelect) {
            this.props.onSelect(hit.entity.id)
          }
        } else if (lastEntity) {
          lastEntity.colorize = lastColorize;
          lastEntity = null;
